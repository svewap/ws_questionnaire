 ...
 *
 */
class Chart extends \TYPO3\CMS\Fluid\View\AbstractTemplateView
{
    ...
    public function getPartialRootPath()
    {
        if ($this->partialRootPath === null && $this->templatePathAndFilename === null) {
            throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('No partial root path has been specified. Use setPartialRootPath().',
                1288094511);
        }
    ...
    protected function getTemplateIdentifier($actionName = null)
    {
        if ($this->templateSource === null) {
            ...
            $templateFilenameWithoutExtension = basename($templatePathAndFilename,
    ...
    protected function getTemplateSource($actionName = null)
    {
        if ($this->templateSource === null && $this->templatePathAndFilename === null) {
            throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('No template has been specified. Use either setTemplateSource() or setTemplatePathAndFilename().',
                1288085266);
        }
        if ($this->templateSource === null) {
            if (!file_exists($this->templatePathAndFilename)) {
                throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Template could not be found at "' . $this->templatePathAndFilename . '".',
                    1288087061);
            }
    ...
    protected function getLayoutSource($layoutName = 'Default')
    {
        ...
        if ($layoutSource === false) {
            throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('"' . $layoutPathAndFilename . '" is not a valid template resource URI.',
                1312215888);
        }
        return $layoutSource;
    }
    ...
    protected function getLayoutPathAndFilename($layoutName = 'Default')
    {
        ...
        if (!is_dir($layoutRootPath)) {
            throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Layout root path "' . $layoutRootPath . '" does not exist.',
                1288092521);
        }
        $possibleLayoutPaths = [];
        ...
        throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Could not load layout file. Tried following paths: "' . implode('", "',
                $possibleLayoutPaths) . '".', 1288092555);
    }
    ...
    protected function getPartialSource($partialName)
    {
        ...
        if ($partialSource === false) {
            throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('"' . $partialPathAndFilename . '" is not a valid template resource URI.',
                1257246929);
        }
    ...
    protected function getPartialPathAndFilename($partialName)
    {
        ...
        if (!is_dir($partialRootPath)) {
            throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Partial root path "' . $partialRootPath . '" does not exist.',
                1288094648);
        }
        ...
        throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Could not load partial file. Tried following paths: "' . implode('", "',
                $possiblePartialPaths) . '".', 1288092555);
    }
    ...
    protected function createIdentifierForFile($pathAndFilename, $prefix)
    {
        ...
        $templateIdentifier = sprintf('Standalone_%s_%s', $prefix,
